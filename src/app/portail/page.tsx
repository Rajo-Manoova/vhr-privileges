...
import CreatePinForm from './_create_pin'
import ChangePinForm from './_change_pin'
...
export default async function PortailPage({
  ...
  switch (result.state) {
    case 'not_found':
      return <PortailForm error="not_found" email={email} />

    case 'no_pin':
      return (
        <CreatePinForm
          email={email}
          memberName={`${result.member.prenom}${result.member.nom ? ' ' + result.member.nom : ''}`}
        />
      )

    case 'pin_required':
      return <PortailForm error="pin_required" email={email} showPin />

    case 'wrong_pin':
      return <PortailForm error="wrong_pin" email={email} showPin />
  ...
  const cfg        = PALIER_CONFIG[palier as Palier]
  const nextPalier = cfg?.next
  const nextSeuil  = nextPalier ? PALIER_SEUILS[nextPalier as Palier] : null
  const pct        = nextSeuil ? Math.round((cumul / nextSeuil) * 100) : 100
  const remaining  = nextSeuil ? nextSeuil - cumul : 0
  const chances    = PALIER_CHANCES[palier as Palier] ?? 1

  return (
    <div style={{ minHeight: '100dvh', background: 'var(--bg-0)', fontFamily: 'var(--font-body)' }}>

      {/* Hero */}
      <div style={{ background: 'var(--brand)', paddingBottom: '5rem', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', zIndex: 1, textAlign: 'center', padding: '1rem 0 0.5rem' }}>
          <div style={{ position: 'relative', width: 160, height: 160, margin: '0 auto 1.25rem' }}>
            ...
            <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
              <div style={{ fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: '1.375rem', letterSpacing: '-0.03em', color: 'white', lineHeight: 1 }}>{cfg?.label}</div>
              <div style={{ fontSize: '0.75rem', color: 'rgba(255,255,255,0.45)', marginTop: '0.25rem', fontWeight: 500 }}>{pct}%</div>
            </div>
          </div>
          ...
          <p style={{ color: 'rgba(255,255,255,0.45)', fontSize: '0.875rem' }}>{member.email}</p>
        </div>
      </div>

      {/* Card */}
      <div style={{ margin: '-3rem 1rem 0', background: 'white', borderRadius: '1.25rem', padding: '1.5rem', boxShadow: '0 8px 32px rgba(15,45,53,0.12)', position: 'relative', zIndex: 2, maxWidth: 480, marginLeft: 'auto', marginRight: 'auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '0.75rem', marginBottom: '1.5rem' }}>
          {[
            ...
            <div key={label} style={{ background: 'var(--bg-0)', borderRadius: '0.75rem', padding: '0.875rem 0.75rem', textAlign: 'center' }}>
              <div style={{ fontSize: '0.6875rem', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.07em', color: 'var(--text-4)', marginBottom: '0.25rem' }}>{label}</div>
              <div style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 'clamp(0.875rem, 2.5vw, 1.125rem)', color: 'var(--text-1)', letterSpacing: '-0.03em' }}>{value}</div>
            </div>
          ))}
        </div>
          ...
          <div style={{ marginBottom: '1.5rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
              <span style={{ fontSize: '0.8125rem', fontWeight: 600, color: 'var(--text-2)' }}>
                Vers <strong>{PALIER_CONFIG[nextPalier as Palier].label}</strong>
              </span>
              <span style={{ fontSize: '0.75rem', color: 'var(--text-4)' }}>{formatAr(remaining)} restants</span>
            </div>
        ...
        {tierRewards.length > 0 && (
          <>
            ...
            <div style={{ marginBottom: '1.25rem' }}>
              <div style={{ fontWeight: 700, fontSize: '0.875rem', color: 'var(--text-1)', marginBottom: '0.75rem' }}>Récompenses de paliers</div>
              {tierRewards.map((r: any, i: number) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '0.625rem', padding: '0.5rem 0', borderBottom: i < tierRewards.length - 1 ? '1px solid var(--border)' : 'none' }}>
                  <span style={{ padding: '0.2rem 0.5rem', borderRadius: 9999, fontSize: '0.6875rem', fontWeight: 700, textTransform: 'uppercase' as const, letterSpacing: '0.05em', background: r.palier === 'vip' ? '#ede9fe' : r.palier === 'or' ? '#fef9c3' : '#e2e8f0', color: r.palier === 'vip' ? '#5b21b6' : r.palier === 'or' ? '#92400e' : '#475569', flexShrink: 0 }}>{r.palier}</span>
                  <span style={{ fontSize: '0.8125rem', color: 'var(--text-2)', flex: 1 }}>{r.lot_nom}</span>
                  <span style={{ fontSize: '0.75rem', fontWeight: 600, color: r.statut === 'delivered' ? '#16a34a' : 'var(--accent)' }}>{r.statut === 'delivered' ? 'Livré ✓' : 'En attente'}</span>
                </div>
              ))}
            </div>
          </>
        ...
        {wins.length > 0 && (
          <>
            <div className="divider" style={{ marginBottom: '1.25rem' }} />
            <div>
              <div style={{ fontWeight: 700, fontSize: '0.875rem', color: 'var(--text-1)', marginBottom: '0.75rem' }}>Tirages gagnés 🏆</div>
              {wins.slice(0, 5).map((w: any, i: number) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '0.625rem', padding: '0.5rem 0', borderBottom: i < Math.min(wins.length, 5) - 1 ? '1px solid var(--border)' : 'none' }}>
                  <div style={{ width: 24, height: 24, borderRadius: '50%', background: '#dcfce7', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>🏆</div>
                  <span style={{ fontSize: '0.8125rem', color: 'var(--text-2)' }}>{w.session_lots?.lots?.nom ?? 'Lot tiré'}</span>
                  <span style={{ fontSize: '0.75rem', color: 'var(--text-4)', marginLeft: 'auto' }}>{new Date(w.confirmed_at).toLocaleDateString('fr-FR')}</span>
                </div>
              ))}
            </div>
          </>
