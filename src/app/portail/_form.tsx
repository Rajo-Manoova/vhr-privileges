...
const ERROR_MESSAGES = {
  not_found:    'Aucun membre trouvé avec cet email.',
  pin_required: 'Ce compte est protégé. Entrez votre PIN.',
  wrong_pin:    'PIN incorrect. Réessayez ou contactez un animateur.',
}
...
export default function PortailForm({ error, email: defaultEmail = '', showPin: defaultShowPin = false }: Props) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (showPin && pin.trim()) params.set('pin', pin.trim())
    ...
    <div style={{
      ...
      <div style={{ width: '100%', maxWidth: 400, textAlign: 'center' }}>
        ...
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.375rem', background: 'rgba(15,45,53,0.06)', borderRadius: 9999, padding: '0.25rem 0.875rem', marginBottom: '1.25rem' }}>
          ...
          <span style={{ fontSize: '0.6875rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: 'var(--accent)' }}>VHR Privilèges</span>
        </div>

        <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(1.75rem, 5vw, 2.25rem)', fontWeight: 900, letterSpacing: '-0.035em', color: 'var(--text-1)', marginBottom: '0.5rem' }}>
          Mon espace
        </h1>
        <p style={{ color: 'var(--text-3)', fontSize: '0.9375rem', marginBottom: '2rem', lineHeight: 1.6 }}>
          Consultez votre profil et progression VHR Privilèges.
        </p>
