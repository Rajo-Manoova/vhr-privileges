...
export default async function RecompensesPage({
  ...
  // Récupérer les gains avec les infos liées
  // On passe par session_lots pour accéder au lot
  ...
  // Compat backward : afficher le label de la session
    ...
    <div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', marginBottom: '1.5rem' }}>
        ...
        <div style={{ display: 'flex', gap: '0.375rem', flexWrap: 'wrap' }}>
          <Link
            href={buildUrl({ session: undefined, page: '1' })}
            style={{
              padding: '0.375rem 0.75rem', borderRadius: 9999,
              fontSize: '0.8125rem', fontWeight: 600, textDecoration: 'none',
              background: sessionId === 'all' ? 'var(--brand)' : 'white',
              color: sessionId === 'all' ? 'white' : 'var(--text-3)',
              border: `1.5px solid ${sessionId === 'all' ? 'var(--brand)' : 'var(--border)'}`,
            }}
          >
            Toutes les sessions
          </Link>
          {(sessions ?? []).map(s => {
            ...
            return (
              <Link
                key={s.id}
                href={buildUrl({ session: s.id, page: '1' })}
                style={{
                  padding: '0.375rem 0.75rem', borderRadius: 9999,
        ...
        <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          {[
            ...
            const active = etape === value
            return (
              <Link
                key={value}
                href={buildUrl({ etape: value === 'all' ? undefined : value, page: '1' })}
                style={{
                  padding: '0.25rem 0.625rem', borderRadius: 9999,
                  fontSize: '0.75rem', fontWeight: 600, textDecoration: 'none', whiteSpace: 'nowrap',
                  background: active ? 'rgba(15,45,53,0.08)' : 'transparent',
          ...
          <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
            ...
            <div style={{
              ...
              {['Membre', 'Lot gagné', 'Session', 'Date'].map(label => (
                <div key={label} style={{ fontSize: '0.6875rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--text-4)' }}>
                  {label}
                </div>
              ))}
            </div>
              ...
              return (
                ...
                >
                  ...
                  <div style={{ minWidth: 0 }}>
                    ...
                    {lot?.valeur_ar && (
                      <span style={{ fontSize: '0.6875rem', color: 'var(--text-4)', marginLeft: '0.375rem' }}>
                  ...
                  <div style={{ fontSize: '0.8125rem', color: 'var(--text-4)', whiteSpace: 'nowrap' }}>
                    {win.confirmed_at
                      ? new Date(win.confirmed_at).toLocaleDateString('fr-FR', {
